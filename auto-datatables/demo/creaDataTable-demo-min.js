(function () {
	...
	var dataTables_bootstrap4 = createCommonjsModule(function (module, exports) {
	...
	*/
	...
	function creaDataTable( $container, options = {}, bs4 = true ) {
	  ...
	  if(bs4) {
	    ...
	    jquery.extend( jquery.fn.DataTable.ext.classes, dt_classes );

	  } else {
	    jquery.extend( true, jquery.fn.dataTable.defaults, dt_config_base );
	  }

	  options = jquery.extend(true, {}, creaDataTable_default_options, options);
	  ...
	  if(options.datatable_options.stateSave && options.dtRender && options.dtRender.bindToForm) {
	    ...
	    options.datatable_options.stateLoadCallback = function(settings) {
	      ...
	      if(_storage && _storage.search_form_data) {
	        let form = jquery('#' + options.dtRender.bindToForm);
	        for( let i in _storage.search_form_data ) {
	          jquery(`[name="${i}"]`, form).val(_storage.search_form_data[i]);
	        }
	  ...
	  if( options.legacy ) { jquery.fn.dataTable.ext.legacy.ajax = true; }
...
//# sourceMappingURL=creaDataTable-demo-min.js.map
